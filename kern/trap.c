...
// IRQs
void _trap_irq_timer();
void _trap_irq_kbd();
void _trap_irq_2();
void _trap_irq_3();
void _trap_irq_serial();

void _trap_irq_5();
void _trap_irq_6();
void _trap_irq_spurious();
void _trap_irq_8();
void _trap_irq_9();
...
trap_init(void)
{
    ...
    //
    size_t i ;
    SETGATE(idt[T_DIVIDE] , 0, GD_KT, _trap_divide_error, 0);
    SETGATE(idt[T_DEBUG]  , 0, GD_KT, _trap_debug_exception, 0);
    SETGATE(idt[T_NMI]    , 0, GD_KT, _trap_non_maskable_interrupt, 0);
    SETGATE(idt[T_BRKPT]  , 0, GD_KT, _trap_breakpoint, 3);
    SETGATE(idt[T_OFLOW]  , 0, GD_KT, _trap_overflow, 0);
    SETGATE(idt[T_BOUND]  , 0, GD_KT, _trap_bounds_check, 0);
    SETGATE(idt[T_ILLOP]  , 0, GD_KT, _trap_illegal_opcode, 0);
    SETGATE(idt[T_DEVICE] , 0, GD_KT, _trap_device_not_available, 0);
    SETGATE(idt[T_DBLFLT] , 0, GD_KT, _trap_double_fault, 0);
    SETGATE(idt[T_TSS]    , 0, GD_KT, _trap_invalid_task_switch_segment, 0);
    SETGATE(idt[T_SEGNP]  , 0, GD_KT, _trap_segment_not_present, 0);
    SETGATE(idt[T_STACK]  , 0, GD_KT, _trap_stack_exception, 0);
    SETGATE(idt[T_GPFLT]  , 0, GD_KT, _trap_general_protection_fault, 0);
    SETGATE(idt[T_PGFLT]  , 0, GD_KT, _trap_page_fault, 0);
    SETGATE(idt[T_FPERR]  , 0, GD_KT, _trap_floating_point_error, 0);
    SETGATE(idt[T_ALIGN]  , 0, GD_KT, _trap_alignment_check, 0);
    ...
    SETGATE(idt[IRQ_OFFSET + 8], 0, GD_KT, _trap_irq_8, 0);
    SETGATE(idt[IRQ_OFFSET + 9], 0, GD_KT, _trap_irq_9, 0);
    SETGATE(idt[IRQ_OFFSET + 10], 0, GD_KT, _trap_irq_10, 0);
    SETGATE(idt[IRQ_OFFSET + 11], 0, GD_KT, _trap_irq_11, 0);
    SETGATE(idt[IRQ_OFFSET + 12], 0, GD_KT, _trap_irq_12, 0);
    SETGATE(idt[IRQ_OFFSET + 13], 0, GD_KT, _trap_irq_13, 0);

    SETGATE(idt[IRQ_OFFSET + IRQ_IDE], 0, GD_KT, _trap_irq_ide, 0);
    SETGATE(idt[IRQ_OFFSET + 15], 0, GD_KT, _trap_irq_15, 0);


    // syscall
    SETGATE(idt[T_SYSCALL], 0, GD_KT, _trap_system_call, 3);

    SETGATE(idt[IRQ_OFFSET + IRQ_ERROR], 0, GD_KT, _trap_irq_error, 0);
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    if ( curenv->env_pgfault_upcall ) {
        ...
        // tf->tf_esp = (uintptr_t) utf - sizeof(struct UTrapframe);
        tf->tf_esp = (uintptr_t) utf ;
        // cprintf("page_fault_handler (kern/trap.c) : run assembly\n");
        env_run(curenv);
    }
